...
for i=1:length(t)         
    ...
    td(i) = targetRange + (targetVel * t(i));
    r_t(i) = 2.0 * td(i)/c;
    
    % *%TODO* :
    %For each time sample we need update the transmitted and
    %received signal. 
    Tx(i) = cos(2 * pi * (fc*t(i) + fmcwSlope * t(i)^2 / 2));
    Rx (i)  =cos(2 * pi * (fc*(t(i) - r_t(i)) + fmcwSlope * (t(i) - r_t(i))^2 / 2));
...
train_cells = grid_size - (2*Gr+1)*(2*Gd+1);
signals = zeros(size(RDM));
[rangeSize, dopplerSize] = size(RDM);

for i = (Tr + Gr + 1) : (rangeSize - (Tr + Gr))
    for j = (Td + Gd + 1) : (dopplerSize - (Td + Gd))
        
        %vector to save each iteration
        noise_level = zeros(1,1);
        ...
        threshold = pow2db(noise_level / train_cells);
